...
import { UIRecommendationConfig } from '../../uiconfigcustom';
...
export class RecommendationOverlay extends Container<ContainerConfig> {
  ...
  constructor(config: ContainerConfig = {}, player: PlayerAPI) {
    super(config);
    let that = this;
    this.paused = false;
    this.replayButton = new HugeReplayButton();
    let closeButton = new Button({cssClass: 'ui-mosaictancabutton bmpui-ui-button'});
    ...
    let labelMosaic = new Label({ cssClass: 'ui-mosaicetiqueta'});
    labelMosaic.setText(i18n.getLocalizer('next'));

    this.config = this.mergeConfig(config, {
      cssClass: 'ui-recommendation-overlay',
      hidden: true,
      components: [labelMosaic, this.replayButton, closeButton],
    }, this.config);
  ...
  configure(player: PlayerAPI, uimanager: UIInstanceManager): void {
    ...
    let setupRecommendations = () => {
      ...
      if (recommendations.length > 0) {
        ...
        this.addComponent(this.replayButton);
      }
    };
...
class RecommendationItem extends Component<RecommendationItemConfig> {
  private buttonEvents = {
    onClick: new EventDispatcher<Button<RecommendationItemConfig>, NoArgs>(),
  };
  ...
  protected toDomElement(): DOM {
    ...
    let itemElement = new DOM('button', {
      'id': this.config.id,
